...
def get_license_key():
    ...
    license_key = '6efe1d21-a0c1-4ffd-9a9c-d22776356956'
    return license_key
...
def generate_unique_hash():
    ...
    return hashlib.sha256(mac.encode()).hexdigest()
        ...
        # If the hash is not the default, just validate normally
        try:
            ...
            if response.status_code == 200 and response.json().get('valid'):
                ...
                logging.warning(f"License validation failed: {response.text}")
                return False
